...
namespace msm {
    ...
    __global__ void bucket_sum(
        ...
        __shared__ Point acc[WarpPerBlock * THREADS_PER_WARP / TPI];
        acc[group_id] = Point::identity();
            ...
            if unlikely(next_key != key || next_window_id != window_id) {
                if unlikely(first) {
                    ...
                    if (initialized.get(window_id, key - 1)) {
                        sum.get(window_id, key - 1) = sum.get(window_id, key - 1) + acc[group_id];
                    } else {
                        sum.get(window_id, key - 1) = acc[group_id];
                ...
                if (initialized.get(next_window_id, next_key - 1) && (next_key != last_key || next_window_id != last_window_id)) {
                    acc[group_id] = sum.get(next_window_id, next_key - 1);
                } else {
                    acc[group_id] = Point::identity();
                }
            }
    ...
    __global__ void reduceBuckets(
        ...
        u32 group_id = threadIdx.x / TPI;
          
        const u32 buckets_per_thread = div_ceil(Config::n_buckets, total_threads_per_window);

        __shared__ Point sum[WarpPerBlock * THREADS_PER_WARP / TPI], sum_of_sums[WarpPerBlock * THREADS_PER_WARP / TPI];
        ...
        __shared__ Point res[WarpPerBlock * THREADS_PER_WARP / TPI];
        res[group_id] = Point::identity();
        bool found_one = false;
        for (int i = 31; i >= 0; i--)
        {
            if (found_one)
              res[group_id] = res[group_id].self_add();
            if ((scale >> i) & 1)
            {
              found_one = true;
        ...
        if(TPI == 1) {
            Point s = sum_of_sums[group_id];
            s = s + s.shuffle_down(16);
            s = s + s.shuffle_down(8);
            s = s + s.shuffle_down(4);
            s = s + s.shuffle_down(2);
            s = s + s.shuffle_down(1);

            if (lane_id == 0) {
                s.store(smem[warp_id]);
            }
            ...
            if (threadIdx.x < WarpPerBlock) {
                s = Point::load(smem[threadIdx.x]);
            } else {
                s = Point::identity();
            }

            // Reduce in warp1
            if constexpr (WarpPerBlock > 16) s = s + s.shuffle_down(16);
            if constexpr (WarpPerBlock > 8) s = s + s.shuffle_down(8);
            if constexpr (WarpPerBlock > 4) s = s + s.shuffle_down(4);
            if constexpr (WarpPerBlock > 2) s = s + s.shuffle_down(2);
            if constexpr (WarpPerBlock > 1) s = s + s.shuffle_down(1);
    ...
    __global__ void precompute_kernel(u32 *points, u64 len) {
        ...
        u32 group_id = threadIdx.x / TPI;
        if (gid >= len) return;
        __shared__ Point p[256 / TPI];
        if((threadIdx.x % TPI) == 0)
            p[group_id] = PointAffine::load(points + gid * PointAffine::N_WORDS).to_point();
        __syncwarp();
        for (u32 i = 1; i < Config::n_precompute; i++) {
            #pragma unroll
            for (u32 j = 0; j < Config::n_windows * Config::s; j++) {
                p[group_id] = p[group_id].self_add();
            }
            if((threadIdx.x % TPI) == 0)
                p[group_id].to_affine().store(points + (gid + i * len) * PointAffine::N_WORDS);
        }
    }
    ...
    cudaError_t MSM<Config, Number, Point, PointAffine>::run(const u32 batches, std::vector<u32*>::const_iterator h_scalers, bool first_run, cudaStream_t stream) {
        ...
        for (int p = begin; p != end; p += stride) {
            ...
            for (int j = 0; j < batches; j++) {
                ...
                bucket_sum<Config, 4, Point, PointAffine><<<grid_size, block_size, 0, stream>>>(
